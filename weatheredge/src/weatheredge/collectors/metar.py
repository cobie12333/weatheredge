...
class MetarCollector:
    ...
    def parse_metar(raw_metar: str) -> Dict[str, Any]:
        ...
        if vis_match:
            if vis_match.group(1):
                result["visibility_m"] = int(vis_match.group(1)) * 1000
            elif vis_match.group(2):
                result["visibility_m"] = float(vis_match.group(2)) * 1609.34

        press_match = re.search(r'(Q|A)(\d{4})\s', raw_metar)
        if press_match:
            unit, value = press_match.groups()
            press_val = int(value)
            result["pressure_hpa"] = press_val if unit == "Q" else round(press_val * 0.0338639, 1)
    ...
    def calculate_derivatives(self, recent_obs: List[Dict]) -> Dict[str, Optional[float]]:
        """Calculate temperature derivatives from recent observations."""
        result = {"dT_dt": None, "d2T_dt2": None, "du_dt": None, "dv_dt": None}
        if len(recent_obs) < 2:
            return result
        ...
        if len(sorted_obs) >= 2:
            ...
            temp1, temp2 = t1.get("temp_c"), t2.get("temp_c")
            ...
            if all(t is not None for t in [temp1, temp2, temp3]):
                try:
                    ...
                    if dt1 > 0 and dt2 > 0:
                        ...
                        result["d2T_dt2"] = round((rate2 - rate1) / ((dt1 + dt2) / 2), 2)
                except (ValueError, KeyError):
                    pass
    ...
    def run(self) -> int:
        ...
        try:
            ...
            derivatives = self.calculate_derivatives([dict(row) for row in recent_rows])
